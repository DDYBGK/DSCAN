...
class GeometricAffineModule(nn.Module):
    """
    论文中的 Geometric Preservation (GP) Module
    结构: Input -> Affine Trans (Learnable Matrix) -> MLP -> Output
    """
...
class GeometricStream(nn.Module):
    ...
    4个阶段的 GP 模块，维度变化: 3 -> 64 -> 128 -> 256 -> 512
    """
...
class Model(nn.Module):
    def __init__(self, points=1024, class_num=40, embed_dim=64, groups=1, res_expansion=1.0,
                 ...
                 # 融合参数
                 geo_ratio=2, sem_ratio=3, fusion_dim=1024,
                 **kwargs):
        ...
        for i in range(len(pre_blocks)):
            ...
            # --- 核心修改：使用传入的 pre_blocks 和 pos_blocks 列表参数 ---
        ...
        # 3. 几何流 (Geometric Stream)
        self.geo_stream = GeometricStream(activation=activation)
        self.geo_output_dim = 512

        # 4. 融合投影层 (Fusion Projection)
        # --- 核心修改：根据传入的 ratio 计算维度 ---
        total_ratio = geo_ratio + sem_ratio

        # 计算分配维度 (ScanObjectNN下: sem=614, geo=410 左右)
...
def DSCAN_ScanObjectNN(num_classes=15, **kwargs) -> Model:
    """
    配置 DSCAN 模型以适应 ScanObjectNN 数据集
    参数修改依据论文:
    1. 层数配置 (Pre/Pos Blocks): [1, 1, 2, 1]
    2. 融合比例 (Geo/Sem): 2:3
    3. 类别数: 15 (ScanObjectNN默认)
    """
...
if __name__ == '__main__':
    # 模拟 ScanObjectNN 输入 (Batch=2, Channels=3, Points=1024)
    data = torch.rand(2, 3, 1024)
    print("===> testing DSCAN for ScanObjectNN (Config: [1,1,2,1], Ratio 2:3) ...")

    # 初始化模型
    model = DSCAN_ScanObjectNN(num_classes=15)

    # 打印融合维度验证
    total_dim = model.fusion_geo_dim + model.fusion_sem_dim
    print(f"融合层配置:")
    print(f"  - 总维度: {total_dim}")
    print(f"  - 语义流 (Sem) 分配: {model.fusion_sem_dim} (约 {model.fusion_sem_dim / total_dim:.0%})")
    print(f"  - 几何流 (Geo) 分配: {model.fusion_geo_dim} (约 {model.fusion_geo_dim / total_dim:.0%})")

    out = model(data)
    print(f"输出形状: {out.shape} (预期: [2, 15])")