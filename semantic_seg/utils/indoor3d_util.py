...
#DATA_PATH = E:\PythonProgram\点云分类与分割框架源码\PointNet++\data\s3dis\Stanford3dDataset_v1.2_Aligned_Version
DATA_PATH = os.path.join(ROOT_DIR, 'data','s3dis', 'Stanford3dDataset_v1.2_Aligned_Version')
g_classes = [x.rstrip() for x in open(os.path.join(BASE_DIR, 'meta/class_names.txt'))]  #类别名称列表
g_class2label = {cls: i for i,cls in enumerate(g_classes)}   #构建类别到标签的映射字典
